...
class DataExtractor_ArchivesSpace(DataExtractor):
    ...
    def find_tree(self, identifier):
        ...
        # TODO: this will need to be re-thought, since the tree endpoint is
        # deprecated
    ...
    def log_fetch_start(self, fetch_type, last_export):
        if last_export > 0:
            logging.info(
                '*** Getting a list of {} modified since %d ***'.format(
                    fetch_type),
                last_export)
        else:
    ...
    def get_updated_resources(self, last_export):
        ...
        for resource in self.repo.resources(
                with_params={'all_ids': True, 'modified_since': last_export}):
            resource_id = resource.uri.split("/")[-1]
            if resource.publish:
                self.save_data_file(
                    resource_id,
                    resource.json(),
                    config.destinations['collections'])
                self.find_tree(resource_id)
            else:
                self.remove_data_file(
                    resource_id, config.destinations['collections'])
                self.remove_data_file(
                    resource_id, config.destinations['trees'])
    ...
    def get_updated_objects(self, last_export):
        ...
        for archival_object in self.repo.archival_objects(
                with_params={'all_ids': True, 'modified_since': last_export}):
            ...
            if archival_object.publish:
                ...
                if breadcrumbs.status_code == 200:
                    self.save_data_file(
                        archival_object_id,
                        breadcrumbs.json(),
                        config.destinations['breadcrumbs'])
            else:
                self.remove_data_file(
                    archival_object_id,
                    config.destinations['objects'])
    ...
    def get_updated_agents(self, last_export):
        ...
        for agent_type, destination_sfx in [
                ...
                ('people', 'people'),
                ('software', 'software')]:
            for agent in getattr(self.aspace.agents, agent_type)(
                    with_params={'all_ids': True, 'modified_since': last_export}):
                agent_id = agent.uri.split("/")[-1]
                if agent.publish:
                    self.save_data_file(
                        agent_id, agent.json(), config.destinations[destination_sfx])
                else:
                    self.remove_data_file(
                        agent_id, config.destinations[destination_sfx])

    def get_updated_subjects(self, last_export):
        """Fetch and save updated subject data."""
        self.log_fetch_start("subjects", last_export)
        for subject in self.aspace.subjects(
                with_params={'all_ids': True, 'modified_since': last_export}):
    ...
    def get_updated_object_data(self, last_export):
        ...
        self.log_fetch_start("objects", last_export)
        updated_data = []
        for archival_object in self.repo.archival_objects.with_params(
                all_ids=True, modified_since=last_export):
