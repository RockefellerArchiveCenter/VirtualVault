...
def test_resources(configure_dirs):
    DataExtractor_ArchivesSpace().get_updated_resources(0)
    assert len(
        listdir(
            join(
                config.DATA_DIR,
                config.destinations["collections"]))) == 0
    assert len(
        listdir(
            join(
                config.DATA_DIR,
                config.destinations["trees"]))) == 0


@aspace_vcr.use_cassette('get_objects.yml')
def test_objects(configure_dirs):
    DataExtractor_ArchivesSpace().get_updated_objects(0)
    assert len(
        listdir(
            join(
                config.DATA_DIR,
                config.destinations["objects"]))) == 0
    assert len(
        listdir(
            join(
                config.DATA_DIR,
                config.destinations["breadcrumbs"]))) == 0


@aspace_vcr.use_cassette('get_subjects.yml')
def test_subjects(configure_dirs):
    DataExtractor_ArchivesSpace().get_updated_subjects(0)
    assert len(
        listdir(
            join(
                config.DATA_DIR,
                config.destinations["subjects"]))) == 0


@aspace_vcr.use_cassette('get_agents.yml')
def test_agents(configure_dirs):
    DataExtractor_ArchivesSpace().get_updated_agents(0)
    for agent_type, expected in [
            ("families", 0), ("organizations", 0), ("people", 0), ("software", 0)]:
        assert len(
            listdir(
                join(
                    config.DATA_DIR,
                    config.destinations[agent_type]))) == expected
